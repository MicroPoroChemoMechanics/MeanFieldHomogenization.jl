...
interaction kernel (see [the theory page](@ref th-interaction)). Four things are
deliberately left open:

- **Anisotropic reference media.** The pair kernel needs a real-space Green
  operator, which is implemented for isotropic references only; an anisotropic
  `C₀` raises an `ArgumentError` naming the limitation rather than silently
  using an isotropic kernel. The missing ingredient exists in exploratory form
  in `echoes_cpp/tests/python/Green/Green.jl` (Pan-Chou closed form for
  transverse isotropy, Barnett-Lothe line integral in general) and would be the
