...
| [A layered spheroid, meshed](axi_layered_spheroid.md) | the same body by Fourier axisymmetric finite elements: calibrated against the two closed forms, taken to nests neither covers, then replaced by a surrogate trained on the cell so the morphology becomes differentiable |
...
The cluster model on cubic arrays and the equivalent inclusion method against a
published table are under **Applications**: each exists to reproduce one paper's
numbers rather than to teach the library.
