# =============================================================================
#  lavergne_model.jl — shared four-scale model of a hydrating blended paste.
#
...
#
#  Included by `scripts/44_lavergne_hydration_micromechanics.jl` and by the
...
#
#  Scale 1 is what distinguishes this model from `pichler_model.jl`, which
...
const LAVERGNE_E_NU = Dict{String, Tuple{Float64, Float64}}(
...
const LAVERGNE_HYDRATES = ["C-S-H", "CH", "AFt", "AFm", "hydrogarnet", "FH3"]
const LAVERGNE_INCLUSIONS = ["anhydrous", "gypsum", "calcite", "silica"]
const LAVERGNE_PORES = ["water", "void"]
...
# and same justification, as `pichler_model.jl`.
const TINY_LAV = 1.0e-6

_stiff(name) = iso_stiffness_E_nu(LAVERGNE_E_NU[name]...)
_stiff(name, ::Type{Float64}) = _stiff(name)
function _stiff(name, ::Type{T}) where {T}
    E, ν = LAVERGNE_E_NU[name]
...
function build_hydrates(f; T = _fT(f))
    f_h = Dict(k => convert(T, get(f, k, zero(T))) for k in LAVERGNE_HYDRATES)
...
function build_foam(f, C_hyd; N::Int = NTHETA_LAVERGNE, ω::Real = ω_hyd, K_water = K_WATER_STATIC)
    T = promote_type(_fT(f), eltype(C_hyd))
    f_hyd = sum(convert(T, get(f, k, 0.0)) for k in LAVERGNE_HYDRATES)
...
function build_paste(f, C_foam)
    ...
    for name in LAVERGNE_INCLUSIONS
...
"""
    lavergne_paste_moduli(f; N, ω, K_water) -> (; K, μ, E, ν)

Effective elastic moduli of a cement paste whose phase volume fractions are `f`,
a `Dict`-like mapping of the family names of `LAVERGNE_HYDRATES`,
`LAVERGNE_INCLUSIONS` and `LAVERGNE_PORES` to fractions summing to one.
...
"""
function lavergne_paste_moduli(
        f; N::Int = NTHETA_LAVERGNE, ω::Real = ω_hyd, K_water = K_WATER_STATIC
    )
    ...
    f_inc = sum(convert(Tf, get(fn, k, 0.0)) for k in LAVERGNE_INCLUSIONS)
...
"""
    lavergne_concrete_moduli(f, f_aggregate; f_air, kwargs...) -> (; K, μ, E, ν)

Effective moduli of a concrete: the paste of [`lavergne_paste_moduli`](@ref)
...
describes the paste.
"""
function lavergne_concrete_moduli(f, f_aggregate; f_air = 0.0, kwargs...)
    m = lavergne_paste_moduli(f; kwargs...)
    _val(m.E) > 0 || return m
